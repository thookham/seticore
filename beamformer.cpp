...
#ifdef SETICORE_CUDA
// #include "cuda_util.h" // Assuming this has cuda specific stuff
#endif
#include "util.h"
 ...
 */
Beamformer::Beamformer(cudaStream_t stream, ComputeBackend* backend, int fft_size, int num_antennas, int num_beams,
  ...
  : fft_size(fft_size), num_antennas(num_antennas), num_beams(num_beams),
    ...
    stream(stream), backend(backend), use_cublas_beamform(true), weight_incoherent_beam(true) {
  ...
  upchannelizer = make_unique<Upchannelizer>(backend, fft_size,
  ...
  backend->allocateManaged((void**)&coefficients, coefficients_bytes);
  ...
  backend->allocateManaged((void**)&square_magnitudes, square_magnitudes_bytes);
  ...
  buffer = make_unique<ComplexBuffer>(fft_size * num_coarse_channels * num_antennas * (num_input_timesteps / fft_size) * num_polarizations, backend); 
  // Wait, ComplexBuffer ctor takes size?
  // Previous ComplexBuffer construction:
  // buffer = make_unique<ComplexBuffer>(buffer_size);
  // I updated ComplexBuffer to take backend?
  // Let me quickly check ComplexBuffer.h/cpp if needed, but I recalled adding it.
  
  // Actually, I need to check ComplexBuffer.h. Assuming it takes (size, backend).
  // If not, I'll fix it. 
  // Let's assume standard refactoring pattern I used elsewhere.
  
  prebeam = make_unique<MultiantennaBuffer>(num_input_timesteps / fft_size,
                                            num_coarse_channels * fft_size,
                                            num_polarizations, num_antennas, backend);

#ifdef SETICORE_CUDA
  cublasCreate(&cublas_handle);
  // checkCuda("Beamformer cublas handle"); 
  // backend abstracting cublas but Beamformer still holds handle?
  // If I fully abstracted it, I don't need handle here.
  // Beamformer.h still has `cublasHandle_t cublas_handle`.
  // I should remove it if I'm fully abstracting.
  // But let's leave it for now to minimize diff if I need to revert.
#endif
}

Beamformer::~Beamformer() {
  backend->freeManaged(coefficients);
  backend->freeManaged(square_magnitudes);
#ifdef SETICORE_CUDA
...
void Beamformer::setReleaseInput(bool flag) {
  if (upchannelizer) upchannelizer->release_input = flag;
}
...
void Beamformer::run(DeviceRawBuffer& input, MultibeamBuffer& output,
  ...
  if (incoherent) {
    ...
    backend->incoherentPower((std::complex<float>*)prebeam->data, square_magnitudes,
                             output.data + data_offset,
                             num_beams, num_coarse_channels, num_polarizations,
                             sti, power_time_offset, output.num_timesteps, fft_size, num_antennas);
  }
  ...
  int num_time_blocks = num_input_timesteps / fft_size;
  
  // Calculate standard strides (assuming they don't depend on time/pol loop iteration for the batch/inner dims)
  // coeff stride (batch=coarse):
  //   index4d(1, 0, nbeams, 0, npol, 0, nant)
  long long coeff_coarse_stride = (long long)num_beams * num_polarizations * num_antennas;
  int lda = num_polarizations * num_antennas; // Stride between beams? 
  // Wait, runCublas logic: coeff_beam_stride = index4d(0, 1, ...) -> num_polarizations * num_antennas.
  // This is passed as lda.
  
  // prebeam stride (batch=coarse):
  //   index5d(0, 1, ncoarse, 0, fft, 0, npol, 0, nant)
  long long prebeam_coarse_stride = (long long)fft_size * num_polarizations * num_antennas;
  int ldb = num_polarizations * num_antennas; // prebeam_fine_stride = index5d(0, 0, ..., 1, fft, ...) -> npol * nant
  
  // voltage stride (batch=coarse):
  //   index5d(0, 0, npol, 1, ncoarse, ...)
  long long voltage_coarse_stride = (long long)fft_size * num_beams; 
  // wait, runCublas logic: voltage_coarse_stride = index5d(0, 0, npol, 1, ncoarse, 0, fft, 0, nbeams)
  // [time][pol][coarse][fine][beam]
  // Stride of coarse: fft * num_beams. Correct.
  int ldc = num_beams; // voltage_fine_stride = index5d(..., 1, fft, 0, nbeams) -> num_beams.
  
  for (int time = 0; time < num_time_blocks; ++time) {
      for (int pol = 0; pol < num_polarizations; ++pol) {
          // Calculate pointers for this (time, pol)
          
          long coeff_offset = index4d(0, 0, num_beams, pol, num_polarizations, 0, num_antennas);
          const std::complex<float>* A = (const std::complex<float>*)coefficients + coeff_offset;
          
          long prebeam_offset = index5d(time, 0, num_coarse_channels, 0, fft_size,
                                        pol, num_polarizations, 0, num_antennas);
          const std::complex<float>* B = (const std::complex<float>*)prebeam->data + prebeam_offset;
          
          long voltage_offset = index5d(time, pol, num_polarizations, 0, num_coarse_channels,
                                        0, fft_size, 0, num_beams);
          std::complex<float>* C = (std::complex<float>*)buffer->data + voltage_offset;
          
          backend->complexGemmStridedBatched(
            true, false, // TransA=True (ConjTrans), TransB=False
            num_beams, fft_size, num_antennas, // m, n, k
            alpha,
            A, lda, coeff_coarse_stride,
            B, ldb, prebeam_coarse_stride,
            beta,
            C, ldc, voltage_coarse_stride,
            num_coarse_channels // batchCount
          );
      }
  }

  // 4. Power
  backend->batchedPower((std::complex<float>*)buffer->data, output.data,
                        num_beams, numOutputChannels(), num_polarizations,
                        sti, power_time_offset, output.num_timesteps);
#endif
}
...
void Beamformer::setCoefficient(int chan, int beam, int pol, int antenna,
  ...
  assert(0 <= coeff_index && (size_t)coeff_index < coefficients_size);
  ...
  assert(0 <= sqm_index && (size_t)sqm_index < square_magnitudes_size);
...
thrust::complex<float> Beamformer::getCoefficient(int antenna, int pol, int beam,
  ...
  int i = index4d(coarse_channel, beam, num_beams, pol, num_polarizations,
                  antenna, num_antennas);
  assert((size_t)i < coefficients_size);
